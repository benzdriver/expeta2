...
export class SemanticMediatorController {
  ...
  async generateTransformationPath(@Body() dto: any) {
  ...
  async executeTransformation(@Body() dto: any) {
  ...
  async validateTransformation(@Body() dto: any) {
  ...
  async getMostUsedPaths(@Query('limit') limit?: string) {
    ...
    const paths = await this.intelligentCache.getMostUsedPaths(limit ? parseInt(limit) : undefined);
  ...
  async getRecentlyUsedPaths(@Query('limit') limit?: string) {
    ...
    const paths = await this.intelligentCache.getRecentlyUsedPaths(limit ? parseInt(limit) : undefined);
  ...
  async requestHumanReview(@Body() dto: RequestHumanReviewDto) {
    ...
    const reviewId = await this.humanInTheLoop.requestHumanReview(
      data,
      context,
      timeout
    );
  ...
  async submitHumanFeedback(@Body() dto: SubmitHumanFeedbackDto) {
    ...
    const success = await this.humanInTheLoop.submitHumanFeedback(
      reviewId,
      feedback,
      metadata
    );
